...
# class EnhancedFrequencyEncoder(nn.Module):
#     def __init__(self, in_channels=3):
#         super(EnhancedFrequencyEncoder, self).__init__()
...
#         magnitude_input = torch.cat(magnitude_features, dim=1)
#         phase_input = torch.cat(phase_features, dim=1)
        
#         # Log-scale magnitude (common practice for frequency analysis)
#         magnitude_input = torch.log(magnitude_input + 1e-10)
        
#         # Apply separate encoders
#         magnitude_encoded = self.magnitude_encoder(magnitude_input)
#         phase_encoded = self.phase_encoder(phase_input)
        
#         # Concatenate and jointly process
#         joint_features = torch.cat([magnitude_encoded, phase_encoded], dim=1)
#         output = self.joint_encoder(joint_features)
